...
# CONFIGURATION
# ===========================
API_BASE_URL = "http://rag-backend.dza7hmatg5c6epcb.eastus.azurecontainer.io:8000"
...
def make_request(
    method: str, endpoint: str, data: dict = None, files: dict = None, auth: bool = True
):
...
def forgot_password(email: str) -> bool:
    ...
    return (
        handle_api_response(response, "📧 Reset link sent to your email!") is not None
    )
...
def change_password(old_password: str, new_password: str) -> bool:
    ...
    return (
        handle_api_response(response, "✅ Password changed successfully!") is not None
    )
...
def login_page():
    ...
    st.markdown(
        "<h3 style='text-align: center; color: gray;'>Login to your account</h3>",
        unsafe_allow_html=True,
    )
...
def signup_page():
    """Signup page UI"""
    st.markdown(
        "<h1 style='text-align: center;'>📝 Create Account</h1>", unsafe_allow_html=True
    )
    st.markdown(
        "<h3 style='text-align: center; color: gray;'>Join us today</h3>",
        unsafe_allow_html=True,
    )
...
def forgot_password_page():
    ...
    st.markdown(
        "<h1 style='text-align: center;'>🔑 Reset Password</h1>", unsafe_allow_html=True
    )
    st.markdown(
        "<h3 style='text-align: center; color: gray;'>Enter your email to receive reset link</h3>",
        unsafe_allow_html=True,
    )
...
def reset_password_page():
    ...
    st.markdown(
        "<h3 style='text-align: center; color: gray;'>Enter your new password</h3>",
        unsafe_allow_html=True,
    )
    ...
    with col2:
        ...
        if not token:
            ...
            token = st.text_input(
                "Reset Token", placeholder="Paste your reset token here"
            )
        else:
...
def chat_page():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "Upload .docx file", type=["docx"], help="Upload a Word document to index"
        )
        ...
        if uploaded_file:
            if st.button("🚀 Upload & Index", use_container_width=True):
                with st.spinner("Processing document..."):
                    ...
                    if result:
                        st.success(
                            f"✅ Indexed {result['indexed']}/{result['total_chunks']} chunks"
                        )
                    ...
                    if response.get("sources"):
                        with st.expander(f"📚 View {response['num_sources']} Sources"):
                            for idx, source in enumerate(response["sources"], 1):
                                st.markdown(
                                    f"**Source {idx}:** `{source['source_file']}`"
                                )
                                st.caption(f"Similarity: {source['similarity']:.2%}")
                                st.text(source["content"])
