...
class TestHealthEndpoints:
    ...
    def test_root_endpoint_returns_healthy(self, fastapi_test_client):
        ...
        with patch('main.embedding_service'), \
...
class TestChatEndpoint:
    ...
    @pytest.mark.unit
    async def test_chat_returns_response(self, fastapi_test_client, sample_chat_request, sample_documents_for_rag):
        """Test that chat endpoint returns a response."""
        with patch('main.embedding_service') as mock_embedding, \
             patch('main.vector_store') as mock_vector, \
            ...
            mock_rag.generate_response.return_value = "The document discusses machine learning and its applications."
    ...
    @pytest.mark.unit
    async def test_chat_no_documents_found(self, fastapi_test_client, sample_chat_request):
    ...
    def test_chat_invalid_top_k(self, fastapi_test_client):
        ...
        with patch('main.embedding_service'), \
            ...
            response = fastapi_test_client.post("/chat", json={
                "query": "What is AI?",
...
class TestUploadDocumentEndpoint:
    ...
    def test_upload_document_returns_indexed_count(self, fastapi_test_client):
        ...
        with patch('main.embedding_service') as mock_embedding, \
             patch('main.vector_store') as mock_vector, \
             patch('main.rag_service'), \
             patch('main.TextProcessor') as mock_processor:
            ...
            response = fastapi_test_client.post(
                "/upload-document",
                files={"file": ("test.docx", test_file_content)}
            )
